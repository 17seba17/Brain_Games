...
const second_task = new SecondTask();//quaaaaaaaaa
let statusTasks=[];
let allTasks = [second_task];///quaaaaaaaaa
let numbertask=1;  ///n-1
setup();
...
function iniziaTask(){
 ...
 let mytask= allTasks[indexchosen];
  ...
  document.getElementById('div'+(indexchosen+numbertask)).classList.replace('hidden', 'visible');////cambiare
  document.getElementById('status'+(indexchosen+numbertask)).textContent = '🕓';////
...
function resetAllTasks(){
  for(let i=0;i<allTasks.length;i++){
    document.getElementById('div'+(i+numbertask)).classList.replace('visible', 'hidden');////cambiare indice
  }
...
document.getElementById('startButton').style.display = 'inline'; 
 ...
 const due = new SecondTask();////quaaaaaaaaa
